...
} from '@/apis/query/customerPackages/useCustomerPackages'
import {
	Package,
	Check,
	X,
...
function derivePlanTerm(plan: {
	...
	if (t === 'quarterly' || t === '3_months' || t === '3months')
		return 'quarterly'
...
export default function CustomerPackages() {
			...
			<div className='space-y-8'>
				...
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
					{[1, 2, 3].map(i => (
						<div
							key={i}
							className='h-96 bg-surface-card rounded-2xl animate-pulse'
						/>
					))}
				</div>
			</div>
	...
	const purchasedRaw =
		(purchasedPackages as any)?.data ?? purchasedPackages ?? []
	...
	const popularPackageId =
		packageData.find(pkg => (pkg.sell_count ?? 0) === maxSellCount)?.id ??
		null
			...
			<div className='space-y-8'>
				...
				<div className='text-center max-w-3xl mx-auto'>
					...
					<p className='text-lg text-text-secondary'>
						Select the package that best fits your needs. Upgrade or
						downgrade anytime.
					</p>
				</div>
				...
				{packageData.length === 0 ? (
					...
					<div className='grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6'>
						{packageData.map((pkg, index) => {
							...
							const isPopular =
								popularPackageId != null &&
								pkg.id === popularPackageId

							// find plan by selectedTerm
							const planByTerm = pkg.plans.find(
								pl => pl.term === selectedTerm
							)
							...
							// Purchased info
							const packagePurchased = purchasedPackageIds.has(
								pkg.id
							)
							const activePlanPurchased =
								!!activePlan &&
								purchasedPlanIds.has(activePlan.id)
									...
									<div className='p-6 md:p-8'>
										...
										<button
											onClick={() => {
												if (!activePlan) {
													toast.error(
														'No valid plan found for this package'
													)
													return
												}
				...
				<div className='mt-16'>
					...
					<div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
						<div className='p-6 bg-surface-card rounded-xl border border-border-subtle hover:border-primary-500 transition-all'>
							...
							<p className='text-text-secondary'>
								Grow your projects seamlessly with our flexible
						...
						<div className='p-6 bg-surface-card rounded-xl border border-border-subtle hover:border-primary-500 transition-all'>
							...
							<p className='text-text-secondary'>
								Get your REST API up and running in minutes, not
								hours or days.
							</p>
						</div>
					</div>
				...
				<div className='mt-16 text-center'>
					...
					<p className='text-text-secondary mb-6'>
						Our support team is here to help you choose the right
						package
					</p>
				...
				{confirmData && (
					<div className='space-y-4'>
						<p className='text-sm text-text-secondary'>
							...
							plan for{' '}
							<span className='font-semibold text-text-primary-sem'>
								${confirmData.price.toFixed(2)} /{' '}
								{confirmData.unitLabel}
							</span>
							.
						</p>
