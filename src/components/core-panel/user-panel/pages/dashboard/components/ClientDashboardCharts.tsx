		...
		<section className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
			...
			<div className='bg-surface-card dark:bg-surface-card-dark rounded-xl border border-border-subtle dark:border-border-input p-4 shadow-sm'>
				...
				{projectChartData.length ? (
						...
						<ResponsiveContainer width='100%' height='100%'>
							<LineChart data={projectChartData}>
								...
								<XAxis
									...
									tick={{
										fontSize: 11,
										fill: 'var(--chart-axis-text, #9ca3af)'
									}}
									stroke={axisStroke}
								/>
								<YAxis
									...
									allowDecimals={false}
								/>
								<Tooltip wrapperClassName='!bg-surface-card dark:!bg-surface-card-dark !border !border-border-subtle dark:!border-border-input !text-text-primary-sem dark:!text-white' />
								<Line
									type='monotone'
									dataKey='projects'
									stroke={lineStroke}
					...
					<p className='text-xs text-text-secondary dark:text-text-tertiary text-center py-10'>
						Project growth data will appear once you start creating
						projects.
					</p>
				)}
			</div>
			...
			<div className='bg-surface-card dark:bg-surface-card-dark rounded-xl border border-border-subtle dark:border-border-input p-4 shadow-sm'>
				...
				{tableStatsData.length ? (
					...
					<p className='text-xs text-text-secondary dark:text-text-tertiary text-center py-10'>
						Table statistics will appear once you create tables in
						your projects.
					</p>
				)}
			</div>
