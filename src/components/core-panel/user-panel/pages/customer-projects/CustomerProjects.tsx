...
} from '@/apis/query/customerProjects/useCutomerProjects'
...
export default function CustomerProjects() {
	...
	const [statusTargetProject, setStatusTargetProject] =
		useState<Project | null>(null)
	const [statusTargetNewStatus, setStatusTargetNewStatus] = useState<
		'active' | 'inactive'
	>('active')
	...
	const {
		...
		submit
	} = usePatchProjectMutation(String(statusTargetProject?.id ?? '0'))
	...
	const stats = useMemo(() => {
		...
		return {
			...
			active: projects.filter((p: Project) => p.status === 'active')
				.length,
			inactive: projects.filter((p: Project) => p.status === 'inactive')
				.length,
			suspended: projects.filter((p: Project) => p.status === 'suspended')
				.length
		}
	}, [projects])
	...
	const handleConfirmStatusChange = async () => {
		...
		try {
			...
			toast.success(
				`Project "${statusTargetProject.name}" is now ${statusTargetNewStatus}`
			)
			mutate()
		} catch (error: any) {
			Swal.fire({
