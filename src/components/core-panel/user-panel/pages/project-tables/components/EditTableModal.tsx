...
export default function EditTableModal({
	...
	const { data, setData, submit, isLoading, errors } = useUpdateTableMutation(
		table?.id || ''
	)
	...
	const buildSchemaJson = (): ColumnSchema[] => {
		return columns.map(col => ({
			...
			max_length:
				col.data_type === 'VARCHAR' ? col.max_length || 255 : null,
	...
	const buildApiEndpoints = (): ApiEndpoint[] => {
		const baseTable = (data.table_name || table?.name || '')
			.toLowerCase()
			.replace(/\s+/g, '_')
		...
		try {
			...
			Swal.fire({
				...
				text:
					...
					'An unexpected error occurred during table update.'
			})
		}
	}
			...
			<form onSubmit={handleSubmit} className='space-y-5'>
				...
				<div className='flex items-start gap-3 pb-4 border-b border-border-subtle dark:border-border-input'>
					...
					<div className='flex-1'>
						...
						<p className='text-sm text-text-secondary mt-0.5'>
							Update table name, columns, and API endpoints
						</p>
					</div>
				</div>
				...
				<div className='bg-primary-50 dark:bg-primary-900/20 border border-primary-100 dark:border-primary-800 rounded-lg p-4'>
					<p className='text-sm text-primary-700 dark:text-primary-300'>
						<strong>Auto-generated:</strong> CRUD API endpoints
						(GET, POST, PUT, DELETE) will be automatically updated
						for your table.
					</p>
				</div>
