...
const PaginatedTable: React.FC<TableProps> = ({
	...
	const toggleSelectAll = () => {
		...
		setSelectedRows(prev =>
			prev.length === sortedData.length ? [] : allSelected
		)
	}
					...
					<tbody>
						{sortedData.length === 0 ? (
							...
							sortedData.map((row, index) => (
								<tr
									key={row.id}
									className={`border-b dark:border-bg_secondary hover:bg-gray-100 dark:hover:bg-gray-800 ${
										index % 2 === 1
								...
								>
									...
									{rowActions && (
										<td className='px-3 py-2 dark:text-text-primary'>
											{rowActions(row)}
										</td>
									)}
								</tr>
			...
			<div className='flex flex-col md:flex-row md:items-center md:justify-between mt-4 gap-2'>
				...
				<div className='flex items-center justify-center gap-2'>
					...
					<p>
						...
						{totalItems === 0 ? 0 : totalPages} | Total {totalItems}{' '}
						records
					</p>
